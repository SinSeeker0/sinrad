...
function defaults(){
  return {
    version:1,
    settings:{retentionDays:30,maxItems:2000,historyCleanupMode:"manual",historyRetentionHours:24,historyStorageMB:1024,freshnessDays:3,feedLayout:"cards"},
    sources:[],
    items:[],
    updatedAt:0
  };
...
function normalizeSource(input){
  ...
  return {
    ...
    platform:platform,
    adapter:cleanText(source.adapter || (platform === "reddit" ? "extension" : ""), 30).toLowerCase(),
    handle:handle,
    ...
    seenPostKeys:Array.from(new Set((Array.isArray(source.seenPostKeys)?source.seenPostKeys:[]).map(function(value){return cleanText(value,80).toLowerCase();}).filter(function(value){return /^[a-z0-9_]{2,21}:[a-z0-9]+$/.test(value);}))).slice(-5000),
    lastSync:safeInteger(source.lastSync, 0, 0, Number.MAX_SAFE_INTEGER),
    syncRequestedAt:safeInteger(source.syncRequestedAt, 0, 0, Number.MAX_SAFE_INTEGER),
...
function normalizeItem(input){
  ...
  return {
    ...
    videoQualityChecked:!!item.videoQualityChecked,
    updatedAt:safeInteger(item.updatedAt, Date.now(), 0, Number.MAX_SAFE_INTEGER),
    read:!!item.read,
    readAt:item.read?safeInteger(item.readAt,item.updatedAt||item.downloadedAt||Date.now(),0,Number.MAX_SAFE_INTEGER):0,
...
class OfflineFeedStore {
  ...
  snapshot(){
    ...
    return JSON.parse(JSON.stringify({
      ...
      items:data.items.slice().sort(function(a,b){return (b.downloadedAt||b.updatedAt||b.date)-(a.downloadedAt||a.updatedAt||a.date);}),
  ...
  configure(settings){
    ...
    if(settings&&["manual","age","storage"].includes(settings.historyCleanupMode))data.settings.historyCleanupMode=settings.historyCleanupMode;
    data.settings.historyRetentionHours=safeInteger(settings&&settings.historyRetentionHours,data.settings.historyRetentionHours||24,1,87600);
    data.settings.historyStorageMB=safeInteger(settings&&settings.historyStorageMB,data.settings.historyStorageMB||1024,100,102400);
    data.settings.freshnessDays=safeInteger(settings&&settings.freshnessDays,data.settings.freshnessDays||3,1,30);
    if(settings&&["cards","scroll"].includes(settings.feedLayout))data.settings.feedLayout=settings.feedLayout;
  ...
  updateSource(id, patch){
    ...
    const limit=data.sources[index].limit,regular=data.items.filter(function(item){return item.sourceId===id&&!item.favorite&&!item.read;}).sort(function(a,b){return (b.updatedAt||b.date)-(a.updatedAt||a.date);}),remove=new Set(regular.slice(limit).map(function(item){return item.id;}));
    if(remove.size){data.items.filter(function(item){return remove.has(item.id);}).forEach((item)=>this._deleteItemFiles(item));data.items=data.items.filter(function(item){return !remove.has(item.id);});}
    data.updatedAt = Date.now();
    this.save();
  ...
  addCapture(input){
    ...
    const result=this.mergeItems("",[{
      sourceKey:key,
      sourceId:cleanText(raw.sourceId,120),
  ...
  updateItem(id, patch){
    ...
    if(!item) return null;
    if(patch && Object.prototype.hasOwnProperty.call(patch, "read")){item.read=!!patch.read;item.readAt=item.read?(item.readAt||Date.now()):0;}
  ...
  replaceItemMedia(id, refs, urls){
    const item=this.load().items.find(function(entry){return entry.id===id;});if(!item)return null;
    const prior=(item.media||[]).slice(),next=(Array.isArray(refs)?refs:[]).map(function(ref){return cleanText(ref,240);}).filter((ref,index,list)=>!!this.resolveMedia(ref)&&list.indexOf(ref)===index).slice(0,20);if(!next.length)return null;
    item.media=next;if(Array.isArray(urls)){item.mediaUrls=urls.slice(0,20);item.videoQualityChecked=true;}item.updatedAt=Date.now();this.data.updatedAt=Date.now();this.save();
    const kept=new Set(itemMediaRefs(item));prior.forEach((ref)=>{if(kept.has(ref))return;try{const target=this.resolveMedia(ref);if(target&&fs.existsSync(target))fs.unlinkSync(target);}catch(_){}});return item;
  }

  removeBrokenExtensionCaptures(){
    const data=this.load(),removed=[],sourceIds=[];
    data.items=data.items.filter(function(item){
      const broken=!!item.sourceId&&item.platform==="reddit"&&item.title==="Reddit - The heart of the internet"&&!item.content&&!(item.media&&item.media.length);
      if(!broken)return true;
      removed.push(item);if(!sourceIds.includes(item.sourceId))sourceIds.push(item.sourceId);return false;
    });
    if(!removed.length)return {removed:0,sourceIds:[]};
    removed.forEach((item)=>{itemMediaRefs(item).forEach((ref)=>{try{const target=this.resolveMedia(ref);if(target&&fs.existsSync(target))fs.unlinkSync(target);}catch(_){}});if(item.captureRef){try{const target=this.resolveCapture(item.captureRef);if(target&&fs.existsSync(target))fs.unlinkSync(target);}catch(_){}}});
    data.updatedAt=Date.now();this.save();return {removed:removed.length,sourceIds:sourceIds};
  }

  _deleteItemFiles(item){
    itemMediaRefs(item).forEach((ref)=>{try{const target=this.resolveMedia(ref);if(target&&fs.existsSync(target))fs.unlinkSync(target);}catch(_){}});
    if(item&&item.captureRef){try{const target=this.resolveCapture(item.captureRef);if(target&&fs.existsSync(target))fs.unlinkSync(target);}catch(_){}}
  }

  removeItem(id){
    const data=this.load(),index=data.items.findIndex(function(item){return item.id===id;});
    if(index<0)return false;
    const item=data.items[index];data.items.splice(index,1);this._deleteItemFiles(item);data.updatedAt=Date.now();this.save();return true;
  }

  takeItems(ids){
    const data=this.load(),wanted=new Set((Array.isArray(ids)?ids:[]).map(String)),taken=[];
    data.items=data.items.filter(function(item){if(!wanted.has(item.id))return true;taken.push(JSON.parse(JSON.stringify(item)));return false;});
    if(taken.length){data.updatedAt=Date.now();this.save();}return taken;
  }

  restoreItems(items){
    const data=this.load(),keys=new Set(data.items.map(function(item){return item.sourceKey;}));let restored=0;
    (Array.isArray(items)?items:[]).forEach(function(raw){const item=normalizeItem(raw);if(keys.has(item.sourceKey))return;data.items.push(item);keys.add(item.sourceKey);restored++;});
    if(restored){data.updatedAt=Date.now();this.save();}return restored;
  }

  purgeItems(items){(Array.isArray(items)?items:[]).forEach((item)=>this._deleteItemFiles(item));}

  takeHistory(){return this.takeItems(this.load().items.filter(function(item){return item.read&&!item.favorite;}).map(function(item){return item.id;}));}

  removeReadItems(sourceIds){
    const data=this.load(),wanted=new Set((Array.isArray(sourceIds)?sourceIds:[]).map(String)),removed=[];
    data.items=data.items.filter(function(item){const drop=!!item.sourceId&&wanted.has(item.sourceId)&&item.read&&!item.favorite;if(drop)removed.push(item);return !drop;});
    if(!removed.length)return {removed:0,sourceIds:Array.from(wanted)};
    removed.forEach((item)=>this._deleteItemFiles(item));data.updatedAt=Date.now();this.save();return {removed:removed.length,sourceIds:Array.from(wanted)};
  }

  _itemStoredBytes(item){
    let total=safeInteger(item&&item.captureSize,0,0,512*1024*1024);itemMediaRefs(item).forEach((ref)=>{try{const target=this.resolveMedia(ref),stat=target&&fs.statSync(target);if(stat&&stat.isFile())total+=stat.size;}catch(_){}});return total;
  }

  cleanupHistory(now){
    const data=this.load(),mode=data.settings.historyCleanupMode||"manual";if(mode==="manual")return {removed:0,total:data.items.length};
    const history=data.items.filter(function(item){return item.read&&!item.favorite;}).sort(function(a,b){return (a.readAt||a.updatedAt)-(b.readAt||b.updatedAt);}),drop=[];
    if(mode==="age"){
      const cutoff=(now||Date.now())-safeInteger(data.settings.historyRetentionHours,24,1,87600)*60*60*1000;history.forEach(function(item){if((item.readAt||item.updatedAt)<cutoff)drop.push(item);});
    }else if(mode==="storage"){
      const sizes=new Map(),limit=safeInteger(data.settings.historyStorageMB,1024,100,102400)*1024*1024;let total=0;data.items.forEach((item)=>{const size=this._itemStoredBytes(item);sizes.set(item.id,size);total+=size;});for(const item of history){if(total<=limit)break;drop.push(item);total-=sizes.get(item.id)||0;}
    }
    if(!drop.length)return {removed:0,total:data.items.length};const ids=new Set(drop.map(function(item){return item.id;}));data.items=data.items.filter(function(item){return !ids.has(item.id);});drop.forEach((item)=>this._deleteItemFiles(item));data.updatedAt=Date.now();this.save();return {removed:drop.length,total:data.items.length};
  }

  cleanupStale(now){
    const data=this.load(),cutoff=(now||Date.now())-safeInteger(data.settings.freshnessDays,3,1,30)*DAY,drop=data.items.filter(function(item){return !item.favorite&&(item.downloadedAt||item.updatedAt||item.date)<cutoff;});
    if(!drop.length)return {removed:0,total:data.items.length,sourceIds:[]};
    const ids=new Set(drop.map(function(item){return item.id;})),sourceIds=Array.from(new Set(drop.map(function(item){return item.sourceId;}).filter(Boolean)));data.items=data.items.filter(function(item){return !ids.has(item.id);});drop.forEach((item)=>this._deleteItemFiles(item));data.updatedAt=Date.now();this.save();return {removed:drop.length,total:data.items.length,sourceIds:sourceIds};
  }
  ...
  _applyItemLimit(){
    ...
    prior.forEach((item)=>{if(!kept.has(item.id))this._deleteItemFiles(item);});
  }

  prune(now){
    const stale=this.cleanupStale(now),history=this.cleanupHistory(now);this._applyItemLimit();if(this.data.items.length!==history.total){this.data.updatedAt=Date.now();this.save();}return {removed:stale.removed+history.removed,total:this.data.items.length,sourceIds:stale.sourceIds};
  }
